...
The correlation coefficient between the `x` and `y` values in the `dino` dataset is `r cor(dino_data$x, dino_data$y)`, whereas the correlation coefficient between the `x` and `y` values in the `star` dataset is `r cor(star_data$x, star_data$y)`. This indicates that the `star` dataset has a slightly less linear relationship than the `dino` relationship, although both datasets have minimal linear correlation between their variables overall.
...
The correlation coefficient between the `x` and `y` values in the `dino` dataset is `r cor(dino_data$x, dino_data$y)`, whereas the correlation coefficient between the `x` and `y` values in the `circle` dataset is `r cor(circle_data$x, circle_data$y)`. This indicates that the `circle` dataset has a slightly more linear relationship than the `dino` relationship, although both datasets have minimal linear correlation between their variables overall.

5.  Finally, let's plot all datasets at once. In order to do this we will make use of facetting.

Facet by the dataset variable, placing the plots in a 3 column grid, and don't add a legend.

```{r all-viz, fig.height = 12, fig.width = 8}
