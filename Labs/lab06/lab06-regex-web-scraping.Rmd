...
```{r counter-pubmed, warning = FALSE}
library(xml2)
...
# Note: We use [1-9] instead of [0-9] in the regex above to subset the ids and reduce the size of our request
...
institution <- stringr::str_extract_all(publications_txt,
               "[[:alpha:]\\s]*University\\sof[[:alpha:]\\s]* | [[:alpha:]\\s]*Institute\\sof[[:alpha:]\\s]*")

institution <- unlist(institution)
head(table(institution), n = 10)
```
...
```{r school-department}
schools_and_deps <- stringr::str_extract_all(publications_txt,
               "[[:alpha:]\\s]*School\\sof[[:alpha:]\\s]* | [[:alpha:]\\s]*Department\\sof[[:alpha:]\\s]*")

schools_and_deps <- unlist(schools_and_deps)
head(table(schools_and_deps), n = 10)
```
...
Of the 360 articles which we have collected, 184 do not have valid abstracts, whereas the other 176 do.
...
All of the 360 articles which we have collected have valid titles.
