...
knitr::kable(head(countries_data, 5),
             caption = "Example of per-country data") %>%
  kable_styling(bootstrap_options = c("striped", "bordered"),
                fixed_thead = TRUE) %>%
  scroll_box(width = "100%")
```
...
```{r basic-plot-1, echo = FALSE, fig.dim = c(11, 8.5)}
...
To answer this question, we will compare each country's average Spotify valence to their happiness. We will compute the correlation between these variables using two different correlation metrics; the Pearson correlation coefficient, and Spearman's rank correlation coefficient.
...
- The set of people who have access to Spotify in each country
